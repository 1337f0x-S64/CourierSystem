...
public class PackagesController {
    ...
    @FXML TableView<Packages>              tablePackages;
    @FXML TableColumn<Packages, Integer>   colId;
    @FXML private TableColumn<Packages, Integer> colUserId;
    @FXML private TableColumn<Packages, String>  colNameUser;
    @FXML private TableColumn<Packages, Double>  colWeight;
    @FXML private TableColumn<Packages, String>  colDescription;
    @FXML private TableColumn<Packages, String>  colTrackingNumber;
    @FXML private TableColumn<Packages, String>  colStatus;
    ...
    public void initialize() {
        ...
        colNameUser.setCellValueFactory(
                cellData -> cellData.getValue().nameUserProperty());
        colWeight.setCellValueFactory(
                cellData -> cellData.getValue().weightProperty().asObject());
        colDescription.setCellValueFactory(
                cellData -> cellData.getValue().descriptionProperty());


        if (colTrackingNumber != null) {
            colTrackingNumber.setCellValueFactory(
                    cellData -> cellData.getValue().trackingNumberProperty());
        }
        if (colStatus != null) {
    ...
    private void loadTable() {
        List<Packages> uiPackages = packageRepository.findAllWithOwnerName();
    ...
    private void addPackage() {
        Windows.newWindowModal(
                "/com/example/courier/views/AddEditPackage.fxml",
                "Add New Package", 240, 300, Optional.of(false), "package.png");
    }
    ...
    private void editPackage() {
        ...
        if (pkg != null) {
            Windows.newWindowModalParams(
                    "/com/example/courier/views/AddEditPackage.fxml",
                    "Edit Package", 240, 300, Optional.of(false), "editor.png",
                    pkg.getId());
        } else {
    ...
    private void deletePackage() {
        ...
        boolean confirmed = Message.showsConfirm(
                "Confirm Deletion",
